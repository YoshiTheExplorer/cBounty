...
export const TaskCards: React.FC<CampaignCardProps> = ({ campaignAddress, index }) => {
    ...
    const dueDate = new Date(Number(data[index]?.dueDate) * 1000);
    const taskDiscription = data[index]?.description;
    ...
    return (
            <div className="flex flex-col justify-between max-w-sm p-6 bg-black border border-slate-200 rounded-lg shadow">
                <div>
                    ...
                    <p className="mb-3 font-normal text-gray-700 dark:text-gray-400">Due: {dueDate.toString()}</p>
                </div>
